...
dnl
AC_MSG_CHECKING([whether to build with jpeg-turbo libjpeg])
...
AS_VAR_IF([with_jpeg], [no], [dnl Not using libjpeg, so no checks are needed
  ...
  AC_MSG_CHECKING([for jpeg-turbo libjpeg via variables])
...
libjpeg not found despite LIBJPEG_CFLAGS and/or LIBJPEG_LIBS being set.
  ...
  AS_VAR_IF([have_libjpeg], [no], [dnl
    PKG_CHECK_MODULES([LIBJPEG], [libjpeg], [dnl
      AC_MSG_CHECKING([linking with jpeg-turbo libjpeg works])
      ...
      LIBJPEG_LIBS="-ljpeg"
      AC_MSG_CHECKING([for jpeg-turbo libjpeg at default location])
      GP_LINK_LIBJPEG_IFELSE([dnl
        have_libjpeg=yes
        AC_MSG_RESULT([yes])
      ], [dnl
  ...
  AS_VAR_IF([have_libjpeg], [no], [dnl
    GP_CONFIG_MSG([JPEG mangling support],
                  [${have_libjpeg} (requires jpeg-turbo libjpeg)])
  ], [dnl
    AC_DEFINE([HAVE_LIBJPEG], [1],
              [define if building with jpeg-turbo libjpeg])
    GP_CONFIG_MSG([JPEG mangling support],
                  [${have_libjpeg}])
  ])
])
