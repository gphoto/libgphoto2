...
int
ultrapocket_getpicture(GPPort *port, GPContext *context, unsigned char **pdata, int *size, const char *filename)
{
   ...
   int            width, height, outsize, result, imgstart;
   smal_img_type  styp;
   int            ptc,pc,pmmhdr_len, id;
#if DO_GAMMA   
...
ultrapocket_getpicsoverview(
    GPPort **pport, GPContext *context,
 ...
 */
int 
ultrapocket_deletefile(GPPort *port, const char *filename)
{
...
int
ultrapocket_deleteall(GPPort **pport)
{
   unsigned char command[0x10];   
   unsigned char retbuf[0x1000];
