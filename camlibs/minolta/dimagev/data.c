...
int dimagev_get_camera_data(dimagev_t *dimagev) {
	...
	if ( ( p = dimagev_make_packet(DIMAGEV_GET_DATA, 1, 0)) == NULL ) {
		if ( dimagev->debug != 0 ) {
			perror("dimagev_get_camera_data::unable to allocate packet");
		}
		return GP_ERROR;
	}
	...
	if ( gpio_write(dimagev->dev, p->buffer, p->length) == GPIO_ERROR ) {
		if ( dimagev->debug != 0 ) {
			perror("dimagev_get_camera_data::unable to write packet");
		}
		return GP_ERROR;
...
int dimagev_send_data(dimagev_t *dimagev) {
	...
	if ( gpio_write(dimagev->dev, p->buffer, p->length) == GPIO_ERROR ) {
		...
		if ( dimagev->debug != 0 ) {
			perror("dimagev_send_data::no response from camera");
		}
		return GP_ERROR;
	}
	...
	if ( gpio_write(dimagev->dev, &char_buffer, 1) == GPIO_ERROR ) {
		if ( dimagev->debug != 0 ) {
			perror("dimagev_send_data::unable to send EOT");
		}
		return GP_ERROR;
	}
